...
describe('Queue component tests', () => {
  ...
  it('The element is added to the queue and that the head and tail cursors are rendered correctly', () => {
    ...
    cy.get('[class*=circle_content]').eq(0).as('firstCircle')
    ...
    cy.get('[class*=circle_content]').eq(1).as('secondCircle')
  ...
  it('Removing an element from the queue', () => {
    ...
    cy.get('[class*=circle_content]').eq(0).should('contain', 'head')

    cy.getByData('remove-button').click()

    cy.get('[class*=circle_content]').eq(0).should('not.contain', 'head')
  })
